...
def do_check_balance():
    ...
    if options.provider == "dialog-new":
        ...
        if balance < options.critical_threshold:
            return (STATUS_CRITICAL, "Balance {} is less than critical threshold of {}".format(balance, options.critical_threshold))
        elif balance < options.warning_threshold:
            return (STATUS_WARNING, "Balance {} is less than warning threshold of {}".format(balance, options.warning_threshold))
        else:
...
def main():
    ...
    try:
        ...
        print_verbose("Checking balance for user {} on {}".format(options.login, options.provider))
